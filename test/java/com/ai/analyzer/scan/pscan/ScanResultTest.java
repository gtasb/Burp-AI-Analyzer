...
class ScanResultTest {
    ...
    class Deduplication {
        ...
        @Test
        @DisplayName("should_generate_key_with_normalized_query")
        void should_generate_key_with_normalized_query() {
            ScanResult result = new ScanResult(1, mockRequestResponse);
            // URL is "https://example.com/api/users?id=1"
            // 新逻辑：query 参数保留（值参与去重），IDOR 探测信号不丢失
            assertThat(result.getDeduplicationKey()).isEqualTo("GET|example.com|https://example.com/api/users|id=1");
        }
    ...
    @Nested
    @DisplayName("主动审计结果合并")
    class FromAuditIssue {

        @Test
        @DisplayName("severity 映射到风险等级")
        void severity_maps_to_risk_level() {
            assertThat(ScanResult.riskLevelFromSeverity(
                    burp.api.montoya.scanner.audit.issues.AuditIssueSeverity.HIGH)).isEqualTo(ScanResult.RiskLevel.HIGH);
            assertThat(ScanResult.riskLevelFromSeverity(
                    burp.api.montoya.scanner.audit.issues.AuditIssueSeverity.MEDIUM)).isEqualTo(ScanResult.RiskLevel.MEDIUM);
            assertThat(ScanResult.riskLevelFromSeverity(
                    burp.api.montoya.scanner.audit.issues.AuditIssueSeverity.LOW)).isEqualTo(ScanResult.RiskLevel.LOW);
            assertThat(ScanResult.riskLevelFromSeverity(
                    burp.api.montoya.scanner.audit.issues.AuditIssueSeverity.INFORMATION)).isEqualTo(ScanResult.RiskLevel.INFO);
            assertThat(ScanResult.riskLevelFromSeverity(null)).isEqualTo(ScanResult.RiskLevel.NONE);
        }

        @Test
        @DisplayName("fromAuditIssue 使用第一个有效请求响应")
        void from_audit_issue_uses_first_valid_request() {
            burp.api.montoya.scanner.audit.issues.AuditIssue issue =
                    mock(burp.api.montoya.scanner.audit.issues.AuditIssue.class);
            when(issue.name()).thenReturn("SQL injection");
            when(issue.detail()).thenReturn("detail text");
            when(issue.remediation()).thenReturn("use prepared statements");
            when(issue.severity()).thenReturn(burp.api.montoya.scanner.audit.issues.AuditIssueSeverity.HIGH);
            when(issue.requestResponses()).thenReturn(java.util.List.of(mockRequestResponse));

            ScanResult result = ScanResult.fromAuditIssue(7, issue);

            assertThat(result.getId()).isEqualTo(7);
            assertThat(result.getStatus()).isEqualTo(ScanResult.ScanStatus.COMPLETED);
            assertThat(result.getRiskLevel()).isEqualTo(ScanResult.RiskLevel.HIGH);
            assertThat(result.getAnalysisResult()).contains("SQL injection");
            assertThat(result.getAnalysisResult()).contains("修复建议");
        }

        @Test
        @DisplayName("无有效请求响应时结果仍生成但为空报文")
        void from_audit_issue_without_request() {
            burp.api.montoya.scanner.audit.issues.AuditIssue issue =
                    mock(burp.api.montoya.scanner.audit.issues.AuditIssue.class);
            when(issue.name()).thenReturn("XSS");
            when(issue.requestResponses()).thenReturn(java.util.Collections.emptyList());
            when(issue.severity()).thenReturn(burp.api.montoya.scanner.audit.issues.AuditIssueSeverity.LOW);

            ScanResult result = ScanResult.fromAuditIssue(9, issue);
