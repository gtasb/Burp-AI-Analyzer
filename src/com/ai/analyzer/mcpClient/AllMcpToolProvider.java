...
public class AllMcpToolProvider {
    ...
    public McpTransport createHttpTransport(String sseUrl) {
        return createHttpTransport(sseUrl, null);
    }

    @SuppressWarnings("deprecation")
    public McpTransport createHttpTransport(String sseUrl, String authorization) {
        Map<String, String> headers = buildAuthorizationHeaders(authorization);
    ...
    public McpTransport createStreamableHttpTransport(String httpUrl) {
        return createStreamableHttpTransport(httpUrl, null);
    }

    public McpTransport createStreamableHttpTransport(String httpUrl, String authorization) {
        Map<String, String> headers = buildAuthorizationHeaders(authorization);
    ...
    /**
     * 创建 WebSocket Transport（用于支持 ws:// 的 MCP Server）
     * @param url MCP 服务器的 WebSocket endpoint URL
     * @return McpTransport 实例
     */
    ...
    /**
     * 根据自定义 MCP 配置创建客户端（支持 SSE / streamableHttp / stdio / websocket）。
     * 失败时抛出异常，由调用方决定是否记录与跳过。
     *
     * @param config 自定义 MCP 配置
     * @return McpClient 实例
     */
    public McpClient createCustomMcpClient(CustomMcpConfig config) {
        if (config == null || !config.isValid()) {
            throw new IllegalArgumentException("自定义 MCP 配置无效: " + (config == null ? "null" : config.getName()));
        }
        ...
        String key = config.getName().trim().replaceAll("[^a-zA-Z0-9._-]+", "-");
        return createMcpClient(transport, key);
    }

    private Map<String, String> buildAuthorizationHeaders(String authorization) {
