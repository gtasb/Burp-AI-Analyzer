    ...
    public static String getDefaultBasePrompt() {
        return """
                ...
                **原则：只有需要人类确认的请求才发送到 Repeater**
                ...
                # 大响应缓存使用规则
                MCP 工具返回的响应可能非常大（例如代理历史、站点地图、响应体搜索、源码/日志等）。
                当某个工具返回超过 ~10KB 时，系统会自动生成 `artifact:<fileId>` 引用代替完整内容。
                - 如需查看完整内容，使用 `read_cache_artifact` 工具按 fileId 读取。
                - 如需按关键词搜索已缓存的大响应，使用 `search_cache_artifact` 工具。
                - 分析时应先读取缓存摘要/开头，判断相关性后再决定是否需要完整内容。
                - 不要把大响应完整复制到最终报告中，仅引用必要片段（<= 500 字符）。
