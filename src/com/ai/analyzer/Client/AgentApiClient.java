...
/*             if (config.isEnableMcp()) {
            ...
            if (config.isEnableCliTool() && (config.isEnableUnrestrictedCliTool()
                    || (config.getCliWhitelist() != null && !config.getCliWhitelist().trim().isEmpty()))) {
        ...
        if (!config.hasAnyMcpEnabled() || mcpToolProvider != null) {
            ...
            return;
                ...
                try {
                    ...
                    // 自定义 MCP 服务器（JSON 配置，支持 SSE / streamableHttp / stdio / websocket）
            ...
            if (config.isEnableMcp()) {
                ...
                for (String tryUrl : new String[]{burpUrl, altUrl}) {
                    try {
                        McpTransport burpTransport = mcpProviderHelper.createHttpTransport(tryUrl, config.getEffectiveBurpMcpAuthorization());
